...
var touchpad = (function () {
  ...
  touchpad.handleTouchstart = function (evt) {
    ...
    for (var i = 0; i < touches.length; i += 1) {
      if (
        ...
        !touches[i].target.classList.contains("touch")
      ) {
        continue;
      }
      foundTouch = true;
  ...
  touchpad.handleTouchmove = function (evt) {
    ...
    for (var i = 0; i < touches.length; i += 1) {
      ...
      if (!moved) {
        var dist = Math.sqrt(
          Math.pow(touches[i].pageX - ongoingTouches[idx].pageXStart, 2) +
            Math.pow(touches[i].pageY - ongoingTouches[idx].pageYStart, 2)
        );
        if (
          ongoingTouches.length > TOUCH_MOVE_THRESHOLD.length ||
    ...
    if (moved && evt.timeStamp - lastEndTimeStamp >= TOUCH_TIMEOUT) {
      if (ongoingTouches.length == 1 || dragging) {
        controller.pointerMove(
          sumX * config.moveSpeed,
          sumY * config.moveSpeed
        );
      } else if (ongoingTouches.length == 2) {
        controller.pointerScroll(
          -sumX * config.scrollSpeed,
          -sumY * config.scrollSpeed,
          false
        );
      }
    }
...
var mouse = (function () {
  ...
  mouse.handleMousemove = function (evt) {
    controller.pointerMove(
      evt.movementX * config.mouseMoveSpeed,
      evt.movementY * config.mouseMoveSpeed
    );
  };
  ...
  mouse.handleWheel = function (evt) {
    if (evt.deltaMode == WheelEvent.DOM_DELTA_PIXEL) {
      controller.pointerScroll(
        evt.deltaX * config.mouseScrollSpeed,
        evt.deltaY * config.mouseScrollSpeed,
        true
      );
    } else if (evt.deltaMode == WheelEvent.DOM_DELTA_LINE) {
      controller.pointerScroll(
        evt.deltaX * 20 * config.mouseScrollSpeed,
        evt.deltaY * 20 * config.mouseScrollSpeed,
        true
      );
    }
  };
...
window.addEventListener("load", function () {
  ...
  ws = new WebSocket(new URL("ws", url).href.replace("http", "ws"));
  ...
  document
    .getElementById("keyboardkeysbutton")
    .addEventListener("click", function () {
      showKeys("keyboard");
    });
  ...
  document.addEventListener("mousedown", function (event) {
    if (
      activeScene != mouseScene &&
      event.buttons == 1 &&
      event.target.classList.contains("touch")
    ) {
      util.requestPointerLock(mouseScene);
    }
  });
  ["touchstart", "touchend", "touchcancel", "touchmove"].forEach(function (
    type
  ) {

...
class MeetPeer extends RTCPeerConnection {
  ...
  changeStream = (newStream, type) => {
    if (this.getSenders().length === 0) return;
    this.getSenders()
...
const MeetJS = function (props) {
  ...
  if (!!props.forceConsole) {
    console.log("forceConsole mode is enabled!");
  ...
  this.userStatusNeeded = props.userStatusNeeded || false;
  ...
  var socketConnect = (userName) => {
    if (this.transport && this.transport.readyState === this.transport.OPEN) {
  ...
  this.getUser = (userName) => {
    ...
    return !!this.users[userName] ? this.users[userName] : initUser(userName);
  };
  ...
  this.on(this.SOCKET_EVENTS.CLOSE_ALL, () => {
    this.getActiveCallUsers().forEach((u) => {
      this.emit(this.SOCKET_EVENTS.BYE, u);
    });
  });
...
MeetJS.prototype.close = function () {
  if (!(this instanceof MeetJS)) return;
...
MeetJS.prototype.isConnected = function () {
  if (!(this instanceof MeetJS)) return;

  return this.transport && this.transport.readyState === WebSocket.OPEN;
};
...
class SignalingChannel extends WebSocket {
  ...
  onopen = () => {
    ...
    if (this.onready) this.onready(this.url);
    this.keepAlive();

    var self = this;

    window.addEventListener("beforeunload", function (e) {
      console.log(self.isRelay, MeetJS.isConnected());
      if (self.isRelay && MeetJS.isConnected()) {
        self.send({
          event: MeetJS.SOCKET_EVENTS.LEFT,
        });
      }
      if (MeetJS.close) MeetJS.close();
    });

    setTimeout(() => {
      if (this.isRelay && MeetJS.isConnected()) {
        this.send({
          event: MeetJS.SOCKET_EVENTS.JOINED,
        });
      }
    }, 5 * this.SECOND);
  };
  ...
  onmessage = (msg) => {
    ...
    if (
      content.event === MeetJS.SOCKET_EVENTS.WELCOME &&
      content.peerName === MeetJS.userName
    ) {
      this.isRelay = false;
      return;
    }
    ...
    if (content.peerName !== MeetJS.userName) {
      if (
        content.event === MeetJS.SOCKET_EVENTS.PING ||
        content.event === MeetJS.SOCKET_EVENTS.JOINED ||
        content.event === MeetJS.SOCKET_EVENTS.LEFT
      ) {
        this.updateActiveUsers(content.peerName);
        if (MeetJS.userStatusNeeded)
          MeetJS.emit(content.event, content.peerName);
        return;
...
const { prototype } = require("./SignalingChannel");
...
Utils.prototype.randomToken = function () {
  return Math.random().toString(36).substr(2);
};

Utils.prototype.isSecure = function () {
